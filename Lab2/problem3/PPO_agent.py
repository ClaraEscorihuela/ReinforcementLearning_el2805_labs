...
class PPOAgent(object):
    ...
    def update(self, buffer, M):
        ...
        g_i = torch.tensor(g_i, dtype=torch.float32) #we need to convert it to a tensor
        ...
        state_grad = torch.tensor(state, requires_grad = True)
        actions = torch.tensor(action, requires_grad = True)
        ...
        for m in range(M):
            ...
            y_without_grad = self.main_critic(torch.tensor(state)).squeeze()
